      ...
      steps {
        script {
            ...
            withCredentials([sshUserPrivateKey(credentialsId: '7d85c306-eb39-4445-ae15-bfeb99f34b33', keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'userName')]) {
                ...
                echo 'env.sandbox_host is below'
                echo env.SANDBOX_HOST
                sshCommand remote: remote, command: "uptime; docker container stop ${env.DOCKER_NAME}; docker container rm ${env.DOCKER_NAME}; docker pull ${env.DOCKER_IMAGE}:${env.DOCKER_LABEL}; docker run -d --name ${env.DOCKER_NAME} --label traefik.frontend.entryPoints=http,https --label traefik.frontend.rule=Host:${env.SANDBOX_HOST}.alphora.com -e FHIR_SERVER_R4='https://cds4cpm-${env.ENVIRONMENT_NAME}.sandbox.alphora.com/fhir' -e FHIR_SERVER_R3='https://cds4cpm-${env.ENVIRONMENT_NAME}.sandbox.alphora.com/fhir' -e FHIR_SERVER_R2='https://cds4cpm-${env.ENVIRONMENT_NAME}.sandbox.alphora.com/fhir' -e BASE_URL='https://${env.SANDBOX_HOST}.alphora.com' ${env.DOCKER_IMAGE}:${env.DOCKER_LABEL}"
            }
        }
      }
    }
